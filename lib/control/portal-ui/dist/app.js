(function () {
  ...
  function normalizeMixer(raw) {
    ...
    return {
      outputGainDb: Number.isFinite(raw.outputGainDb) ? raw.outputGainDb : 6.0,
      outputMute: !!raw.outputMute,
      inputGainDb: Number.isFinite(raw.inputGainDb) ? raw.inputGainDb : 0.0,
      inputMute: !!raw.inputMute,
      jitterFrames: Number.isFinite(raw.jitterFrames) ? raw.jitterFrames : -1,
      inputMode: typeof raw.inputMode === 'string' ? raw.inputMode : 'aux'
    };
  }
  ...
  function isSrc() {
    return state.buildLabel === 'SRC' || state.buildLabel === 'TX' || state.buildLabel === 'COMBO';
  }

  function mixerRoleSection() {
    return isSrc() ? 'mixer-global' : 'mixer-local';
  }

  function applyMixerToUI(m) {
    if (!m) return;
    Object.assign(mixerState, m);

    const gainSliderG = $('output-gain-slider-g');
    const gainDbG = $('output-gain-db-g');
    const muteG = $('output-mute-btn-g');
    const gainSliderL = $('output-gain-slider-l');
    const gainDbL = $('output-gain-db-l');
    const muteL = $('output-mute-btn-l');
    const inputSlider = $('input-gain-slider');
    const inputDb = $('input-gain-db');
    const inputMuteBtn = $('input-mute-btn');

    if (gainSliderG) gainSliderG.value = String(m.outputGainDb);
    if (gainDbG) gainDbG.textContent = formatDb(m.outputGainDb);
    if (muteG) {
      muteG.classList.toggle('mute-btn-active', m.outputMute);
      muteG.setAttribute('aria-pressed', String(m.outputMute));
    }
    if (gainSliderL) gainSliderL.value = String(m.outputGainDb);
    if (gainDbL) gainDbL.textContent = formatDb(m.outputGainDb);
    if (muteL) {
      muteL.classList.toggle('mute-btn-active', m.outputMute);
      muteL.setAttribute('aria-pressed', String(m.outputMute));
    }
    if (inputSlider) inputSlider.value = String(m.inputGainDb);
    if (inputDb) inputDb.textContent = formatDb(m.inputGainDb);
    if (inputMuteBtn) {
      inputMuteBtn.classList.toggle('mute-btn-active', m.inputMute);
      inputMuteBtn.setAttribute('aria-pressed', String(m.inputMute));
    }

    // Latency buttons
    document.querySelectorAll('.latency-btn').forEach((btn) => {
      const frames = parseInt(btn.dataset.frames, 10);
      const active = m.jitterFrames === frames || (m.jitterFrames === -1 && frames === 14);
    ...
    if (auxBtn) auxBtn.classList.toggle('seg-btn-active', m.inputMode !== 'tone');
    if (toneBtn) toneBtn.classList.toggle('seg-btn-active', m.inputMode === 'tone');
  }

  function showMixerRole() {
    const globalEl = $('mixer-global');
    const localEl = $('mixer-local');
    const titleEl = $('mixer-pane-title');
    const src = isSrc();
    if (globalEl) globalEl.style.display = src ? 'block' : 'none';
    if (localEl) localEl.style.display = src ? 'none' : 'block';
    if (titleEl) titleEl.textContent = src ? 'GLOBAL MIXER' : 'LOCAL MIXER';
  }
  ...
  function bindMixerEvents() {
    function bindGainSlider(sliderId, dbId, key) {
      ...
      slider.addEventListener('input', () => {
        ...
        debouncedMixerSend({ [key]: db });
      });
    }

    bindGainSlider('output-gain-slider-g', 'output-gain-db-g', 'outputGainDb');
    bindGainSlider('output-gain-slider-l', 'output-gain-db-l', 'outputGainDb');
    bindGainSlider('input-gain-slider', 'input-gain-db', 'inputGainDb');

    function bindMuteBtn(btnId, key) {
      const btn = $(btnId);
      if (!btn) return;
      btn.addEventListener('click', () => {
        const next = !mixerState[key];
        mixerState[key] = next;
        btn.classList.toggle('mute-btn-active', next);
        btn.setAttribute('aria-pressed', String(next));
        sendMixerPatch({ [key]: next });
        // keep both output mute buttons in sync
        if (key === 'outputMute') {
          const other = btnId === 'output-mute-btn-g' ? $('output-mute-btn-l') : $('output-mute-btn-g');
          if (other) {
            other.classList.toggle('mute-btn-active', next);
            other.setAttribute('aria-pressed', String(next));
          }
        }
      });
    }

    bindMuteBtn('output-mute-btn-g', 'outputMute');
    bindMuteBtn('output-mute-btn-l', 'outputMute');
    bindMuteBtn('input-mute-btn', 'inputMute');
  ...
  async function demoFetch(path, options = {}) {
    ...
    if (path === '/api/mixer' && method === 'GET') {
      return jsonResponse(200, { ...demoRuntime.mixer });
    }
    if (path === '/api/mixer' && method === 'POST') {
      Object.assign(demoRuntime.mixer, payload);
      return jsonResponse(200, { ok: true });
    }
  ...
  function frame(now) {
    ...
    if (fftCanvas) resizeCanvas(fftCanvas, fftCtx);
    drawTopology(t);
    drawFft(t);
  }
