(function () {
  ...
  function normalizeNode(rawNode, fallbackRootMac) {
    ...
    return {
      mac,
      role: typeof rawNode.role === 'string' ? rawNode.role : 'OUT',
      root,
  ...
  function normalizePayload(msg) {
    ...
    return {
      ...
      mixer: msg?.mixer && typeof msg.mixer === 'object' ? msg.mixer : null
    };
  }
  ...
  function updateMixerStatus() {
    const statusEl = $('mixer-status');
    const gainEl = $('mixer-out-gain');
    if (!statusEl || !gainEl) return;
    const m = state.mixer;
    if (!m) {
      statusEl.textContent = 'Mixer status unavailable';
      return;
    }
    const gain = Number.isFinite(m.outGainPct) ? m.outGainPct : 200;
    gainEl.value = String(gain);
    const parts = [`Gain: ${gain}%`];
    if (m.pendingApply) parts.push('Applying...');
    if (m.applied) parts.push('Applied');
  ...
  function ingestPortalPayload(payload) {
    Object.assign(state, normalizePayload(payload));
  ...
  async function demoFetch(path, options = {}) {
    ...
    if (path === '/api/uplink' && method === 'POST') {
      if (typeof payload.enabled !== 'boolean') {
        return jsonResponse(400, { error: 'missing enabled' });
      }
      if (payload.enabled && (!payload.ssid || !String(payload.ssid).trim())) {
        return jsonResponse(400, { error: 'missing ssid' });
      }
    ...
    if (path === '/api/mixer' && method === 'POST') {
      const gain = Number(payload.outGainPct);
      if (!Number.isFinite(gain)) {
        return jsonResponse(400, { error: 'missing outGainPct' });
      }
      if (gain < 0 || gain > 400) {
        return jsonResponse(400, { error: 'invalid outGainPct' });
      }
      demoRuntime.mixer.outGainPct = Math.round(gain);
      demoRuntime.mixer.applied = true;
      demoRuntime.mixer.pendingApply = false;
      demoRuntime.mixer.lastError = '';
      demoRuntime.mixer.updatedMs = Date.now();
      return jsonResponse(200, { ok: true });
    }
    ...
    if (path === '/api/ota' && method === 'POST') {
      ...
      if (!String(payload.url).startsWith('https://')) {
        return jsonResponse(409, { error: 'ota start failed' });
      }
      return jsonResponse(200, { ok: true });
    }
  ...
  async function portalFetch(path, options) {
    ...
    if (controlToken) {
      headers.set('X-SoundMesh-Token', controlToken);
    }
  ...
  async function postMixerGain() {
    const gainEl = $('mixer-out-gain');
    const gain = Number(gainEl?.value ?? '200');
    if (!Number.isFinite(gain) || gain < 0 || gain > 400) {
      window.alert('Output gain must be between 0 and 400.');
      return;
    }
    const res = await portalFetch('/api/mixer', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ outGainPct: Math.round(gain) })
    });
    if (!res.ok) {
      throw new Error(`HTTP ${res.status}`);
    }
  ...
  $('mixer-apply')?.addEventListener('click', async () => {
    try {
      await postMixerGain();
    } catch (err) {
      window.alert(`Failed to apply mixer gain: ${err.message || err}`);
    }
  });
  ...
  function scheduleReconnect() {
    ...
    if (wsAttempts >= 3 && !demoMode) {
      ...
      setConnState('disconnected', 'WS Offline');
      return;
    }
  ...
  function connectWs() {
    ...
    try {
      ...
      if (controlToken) {
        wsUrl.searchParams.set('token', controlToken);
      }
    ...
    ws.onopen = () => {
      wsAttempts = 0;
      demoMode = false;
      if (demoRuntime.intervalId) {
        clearInterval(demoRuntime.intervalId);
        demoRuntime.intervalId = null;
      }
  ...
  function startDemo() {
    ...
    demoRuntime.intervalId = setInterval(() => {
      ingestPortalPayload(buildDemoPayload());
    }, 1000);
  }
